...
After completing Soul Transformation, ring the **Celestial War Bell** on the central skybridge to begin rotating regional incursions. These persistent hunts move between all three domains, arrive in bounded enemy waves, and sometimes culminate in an elite omen. Incursions remain available after the main bosses and provide a repeatable source of Soul Shards and Ascendant Sigils.

Bring those resources back to Scholar Bo to unlock the **Dao Constellations**. Starblade favors critical attacks and boss damage, Moon-Aegis strengthens survival, parries, and ripostes, and Boundless-Step improves movement, qi, and Cloud-Step. Nodes unlock sequentially; one path can be attuned at a time and can be changed freely in the sanctuary.

## Multiplayer

Multiplayer connects automatically while exploring the normal world and disconnects inside the sanctuary or Immortal Realm so hidden-scene players cannot appear as phantom challenge targets. The **Online · Nearby** button lists cultivators within challenge range. Equipment dropped while online appears to everyone in the shard for five minutes and can be claimed once with the normal interact control. Accepted challenges move both players into a separate normalized Arena Realm with pixel fighters, visible attack/parry/dash states, health and cooldown displays, and server-authoritative combat. Local prediction makes movement and action feedback immediate, opponent interpolation removes snapshot judder, and the arena HUD shows measured round-trip latency; the server remains authoritative for positions, hits, and damage. **Leave Realm** and automatic stalled-connection recovery always return the player to the unchanged normal world. Arena outcomes never alter local cultivation progress or the normal-world save, and the solo game remains playable if the service is unavailable.
